...
<< std::endl \
<< " Syntax: " << ExeName << " [options] <<-p|--pid> <pid>|<app> [appargs ...]>" << std::endl \
<< std::endl \
<< "   options:" << std::endl \
...
<< std::endl \
<< "     -s, --shlib <shlib>[,...]" << std::endl \
<< "                         Comma-separated list of shared libraries" << std::endl \
<< "                         which should also be instrumented." << std::endl \
<< std::endl \
<< "     -b, --blacklist <file>" << std::endl \
<< "                         Set path of blacklist file containing" << std::endl \
<< "                         a newline-separated list of functions" << std::endl \
...
Mutator::getFunctions( std::vector<struct InstFunc*> * p_vecInstFuncs )
{
   ...
   for( uint32_t i = 0; i < p_vec_modules->size(); i++ )
   {
	 ...
	 if( statements.size() == 0 || file_name == NULL )
	 {
	    DGOUT( " Skip function '" << function_name << "' (no source code location)" );
	    continue;
	 }
	 else if( constraintModule( file_name ) )
	 {
	    DGOUT( " Skip function '" << function_name << "'" );
	    continue;
	 }
