	      ...
	      uint32_t scltoken )
{
   ...
   theHooks->triggerReadRecordHook( Hooks::Record_Enter, 4,
				    &time, &statetoken, &cpuid, &scltoken );
   ...
   theHooks->triggerWriteRecordHook( Hooks::Record_Enter, 4,
				     &time, &global_statetoken, &cpuid,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_Leave, 4,
				    &time, &statetoken, &cpuid, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_Leave, 4,
				     &time, &global_statetoken, &cpuid,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_FileOperation, 8,
				    &time, &filetoken, &cpuid, &handleid,
				    &operation, &bytes, &duration, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_FileOperation, 8,
				     &newtime, &global_filetoken, &cpuid,
...
Handle_BeginFileOperation( OTF_WStream* wstream,
			   uint64_t time, uint32_t cpuid, uint64_t matchingid,
			   uint32_t scltoken )
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_BeginFileOperation, 4,
				    &time, &cpuid, &matchingid, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_BeginFileOperation, 4,
				     &time, &cpuid, &matchingid,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_EndFileOperation, 7,
				    &time, &cpuid, &filetoken, &matchingid,
				    &handleid, &bytes, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_EndFileOperation, 9,
				     &time, &cpuid, &global_filetoken,
				     &matchingid, &handleid, &operation, &bytes,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_SendMsg, 7,
				    &time, &sender, &receiver, &communicator,
				    &msgtag, &msglength, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_SendMsg, 7,
				     &time, &sender, &receiver,
				     &global_communicator, &msgtag, &msglength,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_RecvMsg, 7,
				    &time, &receiver, &sender, &communicator,
				    &msgtag, &msglength, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_RecvMsg, 7,
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_CollectiveOperation, 9,
				    &time, &process, &functionToken,
				    &communicator, &rootprocess, &sent,
				    &received, &duration, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_CollectiveOperation, 9,
				     &newtime, &process, &global_functionToken,
				     &global_communicator, &rootprocess, &sent,
				     &received, &duration, &global_scltoken );
...
int
Handle_BeginCollectiveOperation( OTF_WStream* wstream,
				 uint64_t time, uint32_t process,
				 uint32_t operation, uint64_t matchid,
				 uint32_t communicator, uint32_t rootprocess,
				 uint64_t sent, uint64_t recvd,
				 uint32_t scltoken )
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_BeginCollectiveOperation, 9,
				    &time, &process, &operation, &matchid,
				    &communicator, &rootprocess, &sent,
				    &recvd, &scltoken );
   ...
   uint32_t global_operation =
      p_tkfac_defcollop->translateLocalToken( mprocess, operation );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_BeginCollectiveOperation, 9,
				     &time, &process, &global_operation,
				     &matchid, &global_communicator,
				     &rootprocess, &sent, &recvd,
				     &global_scltoken );

   int wrrc = OTF_WStream_writeBeginCollectiveOperation( wstream,
							 time, process,
							 global_operation,
							 matchid,
							 global_communicator,
...
int
Handle_EndCollectiveOperation( OTF_WStream* wstream,
			       uint64_t time, uint32_t process,
			       uint64_t matchid )
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_EndCollectiveOperation, 3,
				    &time, &process, &matchid );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_EndCollectiveOperation, 3,
				     &time, &process, &matchid );

   int wrrc = OTF_WStream_writeEndCollectiveOperation( wstream,
						       time, process, matchid );
   return wrrc == 1 ? OTF_RETURN_OK : OTF_RETURN_ABORT;
}
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_RMAPut, 8,
				    &time, &process, &origin, &dest,
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_RMAPut, 8,
				     &time, &process, &origin, &dest,
				     &global_communicator, &tag, &bytes,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_RMAPutRemoteEnd, 8,
				    &time, &process, &origin, &dest,
				    &communicator, &tag, &bytes, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_RMAPutRemoteEnd, 8,
				     &time, &process, &origin, &dest,
				     &global_communicator, &tag, &bytes,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_RMAGet, 8,
				    &time, &process, &origin, &dest,
				    &communicator, &tag, &bytes, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_RMAGet, 8,
				     &time, &process, &origin, &dest,
				     &global_communicator, &tag, &bytes,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_RMAEnd, 6,
				    &time, &process, &remote, &communicator,
				    &tag, &scltoken );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_RMAEnd, 6,
				     &time, &process, &remote,
				     &global_communicator, &tag,
				     &global_scltoken );
...
{
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_Counter, 4,
				    &time, &process, &counter_token, &value );
   ...
   // trigger write record hook
   theHooks->triggerWriteRecordHook( Hooks::Record_Counter, 4,
		     ...
		     uint64_t time, uint32_t process, const char* comment )
{
   ...
   // trigger read record hook
   theHooks->triggerReadRecordHook( Hooks::Record_EventComment, 3,
				    &time, &process, &_comment );
   ...
   if( _comment.compare( "__ETIMESYNC__" ) == 0 )
   {
      wrrc = 1;
#ifdef VT_ETIMESYNC
      if( HaveETimeSync )
   ...
   {
       // trigger write record hook
      theHooks->triggerWriteRecordHook( Hooks::Record_EventComment, 3,
					&time, &process, &_comment );

      wrrc = OTF_WStream_writeEventComment( wstream,
					    time, process, _comment.c_str() );
   }
