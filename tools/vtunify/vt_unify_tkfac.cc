...
TokenFactory::getPackSize()
{
   ...
   if( m_mapLocGlobToken.size() > 0 )
   {
         ...
         it_cpu++ )
      {
         ...
         {
            // m_mapLocGlobToken[]
            CALL_MPI( MPI_Pack_size( (VT_MPI_INT)it_cpu->second->size() * 2,
                                ...
                                VT_MPI_INT * position )
{
   ...
   CALL_MPI( MPI_Pack( &cpu_map_size, 1, MPI_UNSIGNED, buffer, bufferSize,
   ...
   if( cpu_map_size > 0 )
   {
           ...
           it_cpu++ )
      {
         ...
         CALL_MPI( MPI_Pack( &cpu_id, 1, MPI_UNSIGNED, buffer, bufferSize,
                             position, MPI_COMM_WORLD ) );
         ...
         CALL_MPI( MPI_Pack( &token_map_size, 1, MPI_UNSIGNED, buffer,
                             bufferSize, position, MPI_COMM_WORLD ) );
         ...
         if( token_map_size > 0 )
         {
            ...
            CALL_MPI( MPI_Pack( token_map_firsts, (VT_MPI_INT)token_map_size,
                                MPI_UNSIGNED, buffer, bufferSize, position,
                                MPI_COMM_WORLD ) );
                                  ...
                                  VT_MPI_INT * position )
{
   ...
   CALL_MPI( MPI_Unpack( buffer, bufferSize, position, &cpu_map_size, 1,
   ...
   if( cpu_map_size > 0 )
   {
      for( uint32_t i = 0; i < cpu_map_size; i++ )
      {
         // m_mapLocGlobToken[].first
         uint32_t cpu_id;
         CALL_MPI( MPI_Unpack( buffer, bufferSize, position, &cpu_id, 1,
                               MPI_UNSIGNED, MPI_COMM_WORLD ) );

         // m_mapLocGlobToken[].second.size()
         uint32_t token_map_size;
         CALL_MPI( MPI_Unpack( buffer, bufferSize, position, &token_map_size, 1,
                               MPI_UNSIGNED, MPI_COMM_WORLD ) );
         ...
         if( token_map_size > 0 )
         {
            ...
            CALL_MPI( MPI_Unpack( buffer, bufferSize, position,
