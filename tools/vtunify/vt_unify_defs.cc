             ...
             Definitions::DefRec_Base_struct * b )
{
            ...
            b->etype == Definitions::DEF_REC_TYPE__DefProcessGroup )
   {
	 ...
	 return false;
      ...
      // p1 == TYPE_GPU_GROUP && p2 != TYPE_GPU_GROUP
      if( p1->type == Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_GROUP
	  && p2->type != Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_GROUP )
      {
	 return true;
      }
      // p1 != TYPE_GPU_GROUP && p2 == TYPE_GPU_GROUP
      else if(
	 p1->type != Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_GROUP
	 && p2->type == Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_GROUP )
      {
	 return false;
      }
      // p1 == TYPE_GPU_COMM && p2 != TYPE_GPU_COMM
      if( p1->type == Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_COMM
	  && p2->type != Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_COMM )
      {
	 return true;
      }
      // p1 != TYPE_GPU_COMM && p2 == TYPE_GPU_COMM
      else if(
	 p1->type != Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_COMM
	 && p2->type == Definitions::DefRec_DefProcessGroup_struct::TYPE_GPU_COMM )
      {
	 return false;
      }
...
Definitions::run()
{
   ...
   theHooks->triggerPhaseHook( Hooks::Phase_UnifyDefinitions_pre );
   ...
   if( !error )
      theHooks->triggerPhaseHook( Hooks::Phase_UnifyDefinitions_post );
			   ...
			   std::vector<DefRec_Base_struct*> * p_vecGlobDefs )
{
   ...
   for( uint32_t i = 0; i < p_vecLocDefs->size(); i++ )
   {
         ...
         case DEF_REC_TYPE__DefProcessGroup:
	 {
      ...
      // GPU group
			   ...
			   DefRec_DefProcessGroup_struct::TYPE_GPU_GROUP )
        {
        addProc2GPUGroup( p_loc_def_entry->members );
        }
		     ...
		     DefRec_DefProcessGroup_struct::TYPE_MPI_COMM_USER )
	    {
	       addMPIComm( p_loc_def_entry->loccpuid,
			   ...
			   p_loc_def_entry->members );
				   ...
				   DefRec_DefProcessGroup_struct::TYPE_GPU_COMM )
	    {
	    	addGPUComm(p_loc_def_entry->loccpuid,
	 			   p_loc_def_entry->deftoken,
	 			   p_loc_def_entry->members);
   ...
   // add process group for GPUs to global definition records
   addGPUGroup2Global( p_vecGlobDefs );

   // add single process group for all GPU communication
   addGPUComm2Global( p_vecGlobDefs );
			  ...
			  p_vecGlobDefs )
{
   ...
   for( uint32_t i = 0; i < p_vecGlobDefs->size(); i++ )
   {
         ...
         case DEF_REC_TYPE__DefinitionComment:
	 {
	    ...
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefinitionComment, 1,
					      &(p_entry->comment) );
         ...
         case DEF_REC_TYPE__DefCreator:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefCreator, 1,
					      &(p_entry->creator) );
         ...
         case DEF_REC_TYPE__DefTimerResolution:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefTimerResolution, 1,
         ...
         case DEF_REC_TYPE__DefProcess:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefProcess, 3,
					      &(p_entry->deftoken),
					      &(p_entry->name),
					      &(p_entry->parent) );
         ...
         case DEF_REC_TYPE__DefProcessGroup:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefProcessGroup, 4,
					      &(p_entry->deftoken),
					      &(p_entry->name),
					      &n,
					      &array );
         ...
         case DEF_REC_TYPE__DefSclFile:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefSclFile, 2,
					      &(p_entry->deftoken),
					      &(p_entry->filename) );
         ...
         case DEF_REC_TYPE__DefScl:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefScl, 3,
					      &(p_entry->deftoken),
					      &(p_entry->sclfile),
					      &(p_entry->sclline) );
         ...
         case DEF_REC_TYPE__DefFileGroup:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefFileGroup, 2,
					      &(p_entry->deftoken),
					      &(p_entry->name) );
         ...
         case DEF_REC_TYPE__DefFile:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefFile, 3,
					      &(p_entry->deftoken),
					      &(p_entry->name),
					      &(p_entry->group) );
         ...
         case DEF_REC_TYPE__DefFunctionGroup:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefFunctionGroup, 2,
					      &(p_entry->deftoken),
					      &(p_entry->name) );
	 ...
	 { 
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefFunction, 4,
					      &(p_entry->deftoken),
					      &(p_entry->name),
					      &(p_entry->group),
					      &(p_entry->scltoken) );
         ...
         case DEF_REC_TYPE__DefCollectiveOperation:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefCollectiveOperation, 3,
					      &(p_entry->deftoken),
					      &(p_entry->name),
					      &(p_entry->type) );
         ...
         case DEF_REC_TYPE__DefCounterGroup:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefCounterGroup, 2,
					      &(p_entry->deftoken),
					      &(p_entry->name) );
         ...
         case DEF_REC_TYPE__DefCounter:
	 {
	    ...
	    // trigger write record hook
	    theHooks->triggerWriteRecordHook( Hooks::Record_DefCounter, 5,
					      &(p_entry->deftoken),
					      &(p_entry->name),
					      &(p_entry->properties),
...
void
Definitions::addProc2GPUGroup(const std::vector<uint32_t> & vecMembers)
{
  for(uint32_t i = 0; i < vecMembers.size(); i++){
    const uint32_t nodeProc = vecMembers[i];
    // process id already exists for this node?
    std::vector<NodeProc_struct>::iterator it =
      std::find( m_vecGPUGroupProcs.begin(),	m_vecGPUGroupProcs.end(),
		  NodeProc_struct( nodeProc ) );
    if (it == m_vecGPUGroupProcs.end()) {
      m_vecGPUGroupProcs.push_back(NodeProc_struct(nodeProc));
    }
  }
}
...
void
Definitions::addGPUGroup2Global( std::vector<DefRec_Base_struct*> *
				   p_vecGlobDefs )
{
  uint32_t seq_node_group_token = 0xcafebabe;

  std::vector<uint32_t> vec_procids;
  for( uint32_t i = 0; i < m_vecGPUGroupProcs.size(); i++ ){
	  vec_procids.push_back( m_vecGPUGroupProcs[i].procid );
  }

  if(m_vecGPUGroupProcs.size() > 0){
    p_vecGlobDefs->push_back( new DefRec_DefProcessGroup_struct(
				   0,
				   seq_node_group_token++,
				   DefRec_DefProcessGroup_struct::TYPE_GPU_GROUP,
...
// Add each local GPU communicator for later translation
void
Definitions::addGPUComm( const uint32_t proc, const uint32_t defToken,
			                   const std::vector<uint32_t> & vecMembers )
{
    // We could also check here, if the key already exists, although it should not!
    m_mapProcGPUCommTokens[proc] = defToken;

    // vecMembers does not contain duplicates
    for(uint32_t i = 0; i < vecMembers.size(); i++) {
    	m_vecProcGPUCommMembers.push_back(vecMembers[i]);
    }
}

// Create one global GPU communicator and add the translation from the local IDs
void
Definitions::addGPUComm2Global( std::vector<DefRec_Base_struct*> *
				 p_vecGlobDefs )
{
    uint32_t global_token;
    ...
    if (m_vecProcGPUCommMembers.size() == 0 || m_mapProcGPUCommTokens.size() == 0) {
    	return;
    }

    std::map<uint32_t, uint32_t>::iterator it = m_mapProcGPUCommTokens.begin();
    global_token = p_tkfac_defprocessgroup->createGlobalToken(
            it->first,
            it->second,
            "GPU_COMM_GLOBAL",
            m_vecProcGPUCommMembers);
    ...
    it++;
    for( ; it != m_mapProcGPUCommTokens.end(); it++ ) {
    	p_tkfac_defprocessgroup->setTranslation(
    	            it->first,
    	            it->second,
    	            global_token );
