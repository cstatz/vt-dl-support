...
public:
   ...
   struct DefRec_DefProcessGroup_struct : DefRec_Base_struct
   {
      typedef enum { TYPE_NODE, TYPE_MPI_COMM_WORLD, TYPE_MPI_COMM_SELF,
                    TYPE_MPI_COMM_USER, TYPE_OMP_TEAM, TYPE_GPU_COMM,
                    TYPE_GPU_GROUP, TYPE_OTHER}
      ProcessGroupTypeT;
...
private:
   ...
   void addProc2GPUGroup(const std::vector<uint32_t> & vecMembers);

   void addNodeGroups2Global( std::vector<DefRec_Base_struct*> *
			      p_vecGlobDefs );

   void addGPUGroup2Global( std::vector<DefRec_Base_struct*> *
			      p_vecGlobDefs );
   ...
   void addMPIComms2Global( std::vector<DefRec_Base_struct*> *
			    p_vecGlobDefs );
   void addGPUComm( const uint32_t proc, const uint32_t defToken,
   			 const std::vector<uint32_t> & vecMembers );
   void addGPUComm2Global( std::vector<DefRec_Base_struct*> *p_vecGlobDefs );
   ...
   // Members of the GPU process group (GPUs)
   std::vector<NodeProc_struct> m_vecGPUGroupProcs;
   // Members of the GPU communicator (GPUs + corresponding Processes)
   std::vector<uint32_t> m_vecProcGPUCommMembers;
   // Map of the cpuid -> local GPU communicator ID
   std::map<uint32_t, uint32_t> m_mapProcGPUCommTokens;
