...
Synchronization::Synchronization()
{
   ...
   {
      assert( g_vecUnifyCtls[i]->vec_sync_phases.size() > 0 );

      if( !g_vecUnifyCtls[i]->vec_sync_times.empty() )
      {
         assert( !g_vecUnifyCtls[i]->vec_sync_pairs.empty() );
      }
      if( !g_vecUnifyCtls[i]->vec_sync_pairs.empty() )
      {
         assert( !g_vecUnifyCtls[i]->vec_sync_times.empty() );
      }

      // continue if current process doesn't have sync. information
      if( g_vecUnifyCtls[i]->vec_sync_times.empty() &&
          g_vecUnifyCtls[i]->vec_sync_pairs.empty() )
      {
         continue;
      }

      assert( g_vecUnifyCtls[i]->vec_sync_times.size() ==
              g_vecUnifyCtls[i]->vec_sync_pairs.size() );

      // set number of sync. phases, if first process
      if( i == 0 )
      {
         m_uRoundMax = g_vecUnifyCtls[i]->vec_sync_phases.size();
      }
      // check whether number of sync. phases is equal to first process
      else
      {
         assert( g_vecUnifyCtls[i]->vec_sync_phases.size() == m_uRoundMax );
      }

      for( j = 0; j < g_vecUnifyCtls[i]->vec_sync_times.size(); j++ )
      {
         // add timestamps to the global map
         std::pair<uint32_t, uint32_t> key =
            g_vecUnifyCtls[i]->vec_sync_pairs[j];
         SyncTime_struct sync_time =
            g_vecUnifyCtls[i]->vec_sync_times[j];
...
Synchronization::updateSyncParam( uint32_t procId )
{
  ...
  std::vector<SyncPhase_struct> * p_vec_sync_phases = &(g_vecUnifyCtls[it->second]->vec_sync_phases);
...
*/
  ...
  if( round == 1 )
  {
     ...
     for( uint32_t i=0; i < g_vecUnifyCtls.size(); i++ )
     {
        ...
        std::vector<SyncPhase_struct> * p_vec_sync_phases = &(g_vecUnifyCtls[i]->vec_sync_phases);
