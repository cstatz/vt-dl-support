...
Markers::run()
{
   ...
   theHooks->triggerPhaseHook( Hooks::Phase_UnifyMarkers_pre );
   ...
   if( !error )
      theHooks->triggerPhaseHook( Hooks::Phase_UnifyMarkers_post );
				...
				p_vecMarkerDefs )
{
   ...
   for( uint32_t i = 0; i < p_vecMarkerDefs->size(); i++ )
   {
      ...
      theHooks->triggerWriteRecordHook( Hooks::Record_DefMarker, 3,
					&((*p_vecMarkerDefs)[i]->deftoken),
					&((*p_vecMarkerDefs)[i]->name),
