...
Wrapper::run()
{
   ...
   for( i = 0; i < m_pConfig->m_vecComp_SrcFiles.size(); i++ )
   {
      ...
      if( inst_type == INST_TYPE_TAUINST )
      {
         ...
         if( uses_openmp )
         {
            // include path to 'pomp_lib.h'
            m_pConfig->tauinst_addParseArg( m_pConfig->m_sVT_IncDir );
...
Wrapper::showUsageText()
{
   ...
   std::cout << std::endl
             ...
             << std::endl
             << "       tauinst           automatic source code instrumentation by using PDToolkit/TAU"
             << std::endl << std::endl
