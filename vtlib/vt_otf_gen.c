...
typedef enum { BUF_ENTRY_TYPE__DefinitionComment,
               ...
               BUF_ENTRY_TYPE__BeginCollectiveOperation,
               BUF_ENTRY_TYPE__EndCollectiveOperation,
                 ...
                 uint64_t flushBTime, uint64_t* flushETime )
{
  ...
  while(p < gen->buf->pos)
  {
     switch(((VTBuf_Entry_Base*)p)->type)
     {
       ...
       case BUF_ENTRY_TYPE__BeginCollectiveOperation:
       {
       ...
       case BUF_ENTRY_TYPE__EndCollectiveOperation:
       {
  ...
  if(!lastFlush && gen->flushcntr == 0)
  {
    ...
    vt_def_comment(gen->tid,
                   "__VT_COMMENT__ WARNING: This trace is "
...
void VTGen_write_END_FILE_OPERATION(VTGen* gen, uint64_t* time,
       uint32_t fid, uint64_t mid, uint64_t hid, uint32_t op, uint64_t bytes,
       uint32_t sid)
{
  VTGEN_CHECK(gen);
                                            ...
                                            uint32_t sid)
{
  ...
  if (VTGEN_IS_TRACE_ON(gen))
  {
    ...
    new_entry->type   = BUF_ENTRY_TYPE__BeginCollectiveOperation;
    new_entry->length = length;
    new_entry->time   = *time;
    new_entry->rid    = rid;
    new_entry->mid    = mid;
                                          ...
                                          uint64_t mid)
{
  ...
  if (VTGEN_IS_TRACE_ON(gen))
  {
    ...
    new_entry->type   = BUF_ENTRY_TYPE__EndCollectiveOperation;
    new_entry->length = length;
    new_entry->time   = *time;
    new_entry->mid    = mid;
