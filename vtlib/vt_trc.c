...
int vt_num_traces =  1; /* number of processes/MPI-ranks */
int vt_my_trace   =  0; /* current process/MPI-rank id */
int vt_my_ptrace  = -1; /* parent process id */
uint8_t vt_my_trace_is_master = 1; /* 1st MPI-rank on local node? */
uint8_t vt_my_trace_is_disabled = 0; /* MPI-rank disabled? */
...
static void write_def_header(void)
{
  ...
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__ VampirTrace Environment:");
  ...
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MODE: %s", tmp_char);
  ...
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_BUFFER_SIZE: %s",
                 tmp_char);

  /* VT_SYNC_FLUSH */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_SYNC_FLUSH: %s",
                 vt_env_sync_flush() ? "yes" : "no");

  /* VT_SYNC_FLUSH_LEVEL */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_SYNC_FLUSH_LEVEL: %i",
                 vt_env_sync_flush_level());

  /* VT_MAX_FLUSHES */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MAX_FLUSHES: %i", vt_env_max_flushes());

#if defined(VT_METR)
  /* VT_METRICS */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_METRICS: %s",
                 vt_env_metrics() ? vt_env_metrics() : "<not set>");
#endif /* VT_METR */

#if defined(VT_RUSAGE)
  /* VT_RUSAGE */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_RUSAGE: %s",
                 vt_env_rusage() ? vt_env_rusage() : "<not set>");

  /* VT_RUSAGE_INTV */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_RUSAGE_INTV: %i",
                 vt_env_rusage_intv());
#endif /* VT_RUSAGE */

#if (defined(VT_MPI) || defined(VT_HYB))
  /* VT_MPITRACE */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MPITRACE: %s",
                 vt_env_mpitrace() ? "yes" : "no");
#endif /* VT_MPI || VT_HYB */

#if defined(VT_UNIMCI)
  /* VT_MPICHECK */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MPICHECK: %s",
                 vt_env_mpicheck() ? "yes" : "no");

  /* VT_MPICHECK_ERREXIT */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MPICHECK_ERREXIT: %s",
                 vt_env_mpicheck_errexit() ? "yes" : "no");
#endif /* VT_UNIMCI */

#if defined(VT_MEMHOOK)
  /* VT_MEMTRACE */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MEMTRACE: %s",
                 vt_env_memtrace() ? "yes" : "no");

  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_MEMTRACE_MARKER: %s",
                 vt_env_memtrace_marker() ? "yes" : "no");
#endif /* VT_MEMHOOK */

#if defined(VT_GETCPU)
  /* VT_CPUIDTRACE */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_CPUIDTRACE: %s",
                 vt_env_cpuidtrace() ? "yes" : "no");
#endif /* VT_GETCPU */

#if defined(VT_IOWRAP)
  /* VT_IOTRACE */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_IOTRACE: %s",
                 vt_env_iotrace() ? "yes" : "no");
#endif /* VT_IOWRAP */

#if defined(VT_CUDARTWRAP)
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_CUDARTTRACE: %s",
                 vt_env_cudarttrace() ? "yes" : "no");
#endif /* VT_CUDARTWRAP */

#if (defined (VT_ETIMESYNC) && TIMER_IS_GLOBAL == 0)
  /* VT_ETIMESYNC */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_ETIMESYNC: %s",
                 vt_env_etimesync() ? "yes" : "no");

  /* VT_ETIMESYNC_INTV */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_ETIMESYNC_INTV: %i",
                 vt_env_etimesync_intv());
#endif /* VT_ETIMESYNC && !TIMER_IS_GLOBAL */

#if defined(VT_THRD_PTHREAD)
  /* VT_PTHREAD_REUSE */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_PTHREAD_REUSE: %s",
                 vt_env_pthread_reuse() ? "yes" : "no");
#endif /* VT_THRD_PTHREAD */

  /* VT_FILER_SPEC */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_FILTER_SPEC: %s",
                 vt_env_filter_spec() ? vt_env_filter_spec() : "<not set>");

  /* VT_GROUPS_SPEC */
  vt_def_comment(VT_MASTER_THREAD, "__VT_COMMENT__  VT_GROUPS_SPEC: %s",
...
static void write_uctl_file(void)
{
  ...
  for (i = 0; i < (int)VTThrdn; i++)
  {
    sprintf(uctl_data + strlen(uctl_data), "%x%s:",
            65536*i+(vt_my_trace+1), vt_my_trace_is_disabled ? "!" : "");
  }
...
static void unify_traces(void)
{
  ...
  if (!vt_env_compression()) argv[argc++] = strdup("-c");
...
void vt_open()
{
  ...
  /* write start-time as comment to definitions */
  {
    ...
    vt_def_comment(VT_MASTER_THREAD, "__STARTTIME__ %llu",
...
# if (!defined(VT_MPI) && !defined(VT_HYB))
  ...
  {
    vt_plugin_cntr_thread_init(VTThrdv[0]);
...
void vt_close()
{
  ...
  /* write stop-time as comment to definitions */
  {
    ...
    vt_def_comment(VT_MASTER_THREAD, "__STOPTIME__ %llu",
...
uint8_t vt_is_trace_on(uint32_t tid)
{
  ...
  return (VTTHRD_TRACE_STATUS(VTThrdv[tid]) == VT_TRACE_ON) ? 1 : 0;
}
...
#if defined(VT_PLUGIN_CNTR)
  /* update synchronous plugin values */
...

  {
    ...
    if (filter_deffile)
    {
      if (!RFG_Regions_readFilterDefFile(VTThrdv[0]->rfg_regions,
                                         (int)myrank, &vt_my_trace_is_disabled))
      {
        vt_error_msg("Could not read region filter specification file");
      }
                     ...
                     uint32_t cid)
{
  ...
  for(i = 0; i < grpc; i++){
    grpv[i] = grpv[i] * 65536 + vt_my_trace + 1;
  }
